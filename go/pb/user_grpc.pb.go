...
func (c *userServiceClient) AddUsersStreamingBoth(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUsersStreamingBothClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUsersStreamingBoth", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUsersStreamingBothClient{stream}
	return x, nil
}

type UserService_AddUsersStreamingBothClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUsersStreamingBothClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUsersStreamingBothClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUsersStreamingBothClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUsersStreamingBoth(UserService_AddUsersStreamingBothServer) error
...
func (UnimplementedUserServiceServer) AddUsersStreamingBoth(UserService_AddUsersStreamingBothServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUsersStreamingBoth not implemented")
}
...
func _UserService_AddUsersStreamingBoth_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUsersStreamingBoth(&userServiceAddUsersStreamingBothServer{stream})
}

type UserService_AddUsersStreamingBothServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUsersStreamingBothServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUsersStreamingBothServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUsersStreamingBothServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUsersStreamingBoth",
			Handler:       _UserService_AddUsersStreamingBoth_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
